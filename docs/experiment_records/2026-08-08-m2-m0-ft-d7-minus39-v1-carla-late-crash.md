...
M0-FT 初始化已证明 1,132 个 state tensor 与 2026 年 1 月 M0 epoch26 逐值相同。本轮进一步将 v1 的 10 个 pipeline-valid attempt 与 2026-07-23 冻结 M0 D7 中相同 route×seed 对齐。除了 state 等价的 checkpoint 包装路径、协议/结果目录和已审计的评测后清理修复，驾驶输入与环境参数一致。

10 个配对 attempt 的 `new-old` 均值为 DS `+0.048578`、RC `+4.220055 pp`、IS `+0.008475`，Leaderboard 终止状态 8/10 一致。其中 route18 的三个 seed 仍全部在约 3% RC 偏离路线；route30 seed0 仍在约 12% RC 偏离，说明这两类失败已存在于原始 M0，不是视觉 checkpoint 包装引入的新回归。

route6 的单 seed DS 波动较大（差值 `-22.171641` 至 `+31.656256`），显示背景交通闭环并非逐次完全确定；因此该对齐只支持“没有 checkpoint 损坏的证据”，不代替完整 v2 的配对统计。
