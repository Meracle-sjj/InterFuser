...
- **Q3（对照，待轻背景锚点跑完后）**：0 背景车配置下同路线是否恢复通行？
...
3. 随后轻背景锚点（0 背景车，configs/thesis/interfuser_v9_d7_{b0,v}_light_v1.json）双臂 seed0；
4. 进度文件 `/tmp/v9_d7_chain_progress.log`；各 run 日志 `/tmp/m2-v9-d7-*.log`。

**决策点（等视觉判定 + 全矩阵）**：
- 若 Q1/Q2 成立（堵车）：D7 主表用轻背景锚点做论文 §4.4 主数字，重背景作为压力附录（配对 Δ 仍有效）；
