...
carla_dataset.py: 从显式 dataset index 解析 CARLA 多视角 RGB/LiDAR/测量标签，以独立 LiDAR y 轴、旧 yaw/CARLA 0.9.16 compass 导航 frame 和缺失字段策略生成坐标同构的 InterFuser 样本。
...
heatmap_utils.py: 生成 waypoint/参与者热力图，并复用 CarlaMVDetDataset 冻结的旋转矩阵避免轨迹目标坐标分叉。
