...
test_evaluation_runtime_resources.py: 验证 runner 能回收包装进程已退出但子进程仍存活的完整 POSIX 进程组。
...
test_run_thesis_baseline.py: 验证 D7 runner 的路线拆分、CARLA 原生 RPC 崩溃隔离、结果解析、资源门禁和 pipeline-invalid 立即终止。
