...
architecture_pretraining.py: v9 架构内语义预训练库——逐字移植原生驾驶损失（MVTL1/WaypointL1/CE 六项组合）、契约哈希装载、hook 语义头与 epoch 循环。
run_architecture_pretraining.py: v9 阶段一入口——官方 B0 全模型可训，驾驶+辅助语义联合训练，epoch 选择以 validation 驾驶损失为准，产出 Stage 2 初始化 checkpoint。
